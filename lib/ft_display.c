...
/*   Updated: 2020/11/28 15:34:22 by thoberth         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    ft_display(t_list_map *map)
{
    ...
    {
        tmp = i + 4;
        //ft_putnbr(map->ray.dist[a]);
        //ft_putnbr(map->ray.dist_to_pp);
        if (map->ray.dist[a] < 1)
            map->ray.dist[a] = 1;
        map->dis.t_wall = (map->map.Tcub / map->ray.dist[a]) * map->ray.dist_to_pp;
        map->dis.t_fc = (map->map.reso[1] - map->dis.t_wall) / 2;
        map->dis.t_fc = map->dis.t_fc < 0 ? 0 : map->dis.t_fc;
        //ft_putnbr(map->dis.t_wall);
        //ft_putnbr(map->dis.t_fc);
        //ft_putchar('\n');
        if (map->dis.t_wall + (map->dis.t_fc * 2) < map->map.reso[1])
            map->dis.t_wall++;
        i = (map->dis.t_wall > map->map.reso[1] ? i : ft_display_fctex(map, 1, i));
        i = ft_texture(map, a, i);
        i = (map->dis.t_wall > map->map.reso[1] ? i : ft_display_fctex(map, 0, i));
        i = tmp;
        a++;
    }
}